...
def draw_chart(x_label, data):
    """Build a simple bar chart and return notebook-rendered HTML.

    Args:
        x_label (list): Labels for the x-axis categories.
        data (list): Numeric values aligned with `x_label`.

    Returns:
        Any: Notebook render object produced by `chart.render_notebook()`.
    """
    chart = (
        Bar()
        .add_xaxis(x_label)
